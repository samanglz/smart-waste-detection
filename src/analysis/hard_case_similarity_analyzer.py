...
class HardCaseSimilarityAnalyzer:
    ...
    def generate_report(
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            ...
            f.write("=" * 60 + "\n")
            if avg_margin > 0.05:
                f.write("⚠️  Most hard cases are closer to the PREDICTED class in embedding space.\n")
                f.write("   → The errors are likely driven by DATA SIMILARITY.\n")
                f.write("   → Consider improving dataset diversity or targeted augmentation.\n")

            elif avg_margin < -0.05:
                f.write("⚠️  Most hard cases are closer to the GROUND-TRUTH class in embedding space.\n")
                f.write("   → The errors are likely driven by MODEL BEHAVIOR.\n")
                f.write("   → Consider improving the training strategy or classification loss.\n")

            else:
                f.write("ℹ️  Hard cases are balanced between predicted and ground-truth similarity.\n")
                f.write("   → Both data similarity and model behavior are likely contributing.\n")
